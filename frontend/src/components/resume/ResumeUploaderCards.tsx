...
import type { CompareResult, ExperienceItem } from "@/types/resume";
    ...
    const CompactUploader = (
        <Card className="w-full mb-4">
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <Button size="sm" variant="ghost" onClick={() => setCollapsed(false)}>Edit</Button>
                    </div>
                </CardTitle>
            </CardHeader>
            <CardContent>
            ...
            <CardFooter className="flex justify-end">
                ...
                <div className="flex items-center gap-2">
                    <Button size="sm" onClick={handleUpload} disabled={!file || status === "uploading" || status === "processing"}>
                        {status === "uploading" || status === "processing" ? <IconSpinner /> : "Re-run"}
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => handleFile(null)}>Remove</Button>
                </div>
            </CardFooter>
        </Card>
    );
    ...
    return (
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-6 lg:px-10">
            ...
            <div className={collapsed ? "lg:col-span-1" : "lg:col-span-2"}>
                {collapsed ? (
                    ...
                    >
                        <div className="text-center">
                            ...
                            <div className="text-sm text-muted mb-4">Only DOCX currently. We extract skills, experience and generate insights.</div>
                            ...
                            <input ref={inputRef} type="file" accept=".docx" onChange={onChange} className="hidden" />
                        </div>
                    </div>
                )}
                ...
                <div className="mt-4">
                    {file ? (
                        <Card className="relative"> {/* ensure positioned parent */}
                            <CardHeader>
