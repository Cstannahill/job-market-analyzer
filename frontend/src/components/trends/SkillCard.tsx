...
export default function SkillCard({ skill, onClick, isSelected }: Props) {
    ...
    const baseClass = "relative w-full text-left rounded-xl p-1 bg-gradient-to-r from-transparent via-purple-700/12 to-transparent hover:via-purple-600/20 transition";
    const activeClass = !active ? baseClass : `${baseClass} ring-2 ring-purple-500/40 hover:via-purple-500/30`;
    useEffect(() => {
        if (isSelected && skill.id === isSelected.id) {
            setActive(true);
        } else {
            setActive(false);
        }
